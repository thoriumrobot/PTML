...
for fname in files_sorted:
  ...
  if len(json_data)<2:
    ...
    if os.path.exists(modeldir+"best_model_"+cluster+".pkl"):
            with open(modeldir+"best_model_"+cluster+".pkl",'rb') as f:
                checkpoint = torch.load(f)
                model.load_state_dict(checkpoint)
                model = model.to(device)
    elif tempidx>0:
        if os.path.exists(modeldir+"best_model_"+custom_order[tempidx-1]+".pkl"):
            with open(modeldir+"best_model_"+custom_order[tempidx-1]+".pkl",'rb') as f:
                checkpoint = torch.load(f)
                model.load_state_dict(checkpoint)
                model = model.to(device)
    ...
    for epoch in range(200):
        ...
        epoch_loss = total_loss / len(graphs)
        print(f'Epoch {epoch}, Loss: {epoch_loss}')

        if epoch_loss < best_loss:
            best_loss = epoch_loss
            torch.save(model.state_dict(), os.path.join(modeldir, f"best_model_{cluster}.pkl"))
