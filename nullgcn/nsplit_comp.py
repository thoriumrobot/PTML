...
def process_ast(ast):
    ...
    for src, dsts in ast['adjacencyList'].items():
        src = int(src)  # Convert src to integer
        for dst in dsts:
            dst = int(dst)  # Convert dst to integer
...
for fname in files_sorted:
    ...
    with open(fname, "r") as file:
        ...
        while True:
            ...
            # When we have 5 ASTs or reach the end of the file, process them
            if len(asts) == 5 or file.tell() == os.fstat(file.fileno()).st_size:
                ...
                # Clear the list for the next batch of ASTs
                asts = []
