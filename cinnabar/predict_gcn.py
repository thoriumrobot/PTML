...
def process_ast(ast):
    ...
    # Prepare node features for all nodes
    node_features = torch.tensor([multi_hot_encode(node['type'], types) for node in ast['nodes']], dtype=torch.float)

    # Create a mask for nodes of interest
    mask = torch.tensor([i for i, node in enumerate(ast['nodes']) if any(nt in nodeList for nt in node['type'])], dtype=torch.long)

    # Labels can be included if necessary, but they won't be used in prediction
    labels = torch.tensor([node['nullable'] for node in ast['nodes']], dtype=torch.long)

    return Data(x=node_features, edge_index=edge_index, y=labels, mask=mask)
...
def predict_new_asts(model_path, new_asts, types, device):
    ...
    # Skip prediction if there are no valid graphs
